...
import org.javaee7.so.controller.exceptions.IllegalOrphanException;
    ...
    public void create(MicroMarket microMarket) throws PreexistingEntityException, Exception {
        ...
        try {
            ...
            for (Customer customerListCustomer : microMarket.getCustomerList()) {
                MicroMarket oldMicroMarketOfCustomerListCustomer = customerListCustomer.getMicroMarket();
                customerListCustomer.setMicroMarket(microMarket);
                customerListCustomer = em.merge(customerListCustomer);
                if (oldMicroMarketOfCustomerListCustomer != null) {
                    oldMicroMarketOfCustomerListCustomer.getCustomerList().remove(customerListCustomer);
                    oldMicroMarketOfCustomerListCustomer = em.merge(oldMicroMarketOfCustomerListCustomer);
                }
            }
        ...
        try {
            ...
            for (Customer customerListOldCustomer : customerListOld) {
                if (!customerListNew.contains(customerListOldCustomer)) {
                    ...
                    illegalOrphanMessages.add("You must retain Customer " + customerListOldCustomer + " since its microMarket field is not nullable.");
                }
            }
            if (illegalOrphanMessages != null) {
            ...
            for (Customer customerListNewCustomer : customerListNew) {
                if (!customerListOld.contains(customerListNewCustomer)) {
                    MicroMarket oldMicroMarketOfCustomerListNewCustomer = customerListNewCustomer.getMicroMarket();
                    customerListNewCustomer.setMicroMarket(microMarket);
                    customerListNewCustomer = em.merge(customerListNewCustomer);
                    if (oldMicroMarketOfCustomerListNewCustomer != null && !oldMicroMarketOfCustomerListNewCustomer.equals(microMarket)) {
                        oldMicroMarketOfCustomerListNewCustomer.getCustomerList().remove(customerListNewCustomer);
                        oldMicroMarketOfCustomerListNewCustomer = em.merge(oldMicroMarketOfCustomerListNewCustomer);
                    }
                }
            }
            em.getTransaction().commit();
    ...
    public void destroy(String id) throws IllegalOrphanException, NonexistentEntityException {
        ...
        try {
            ...
            for (Customer customerListOrphanCheckCustomer : customerListOrphanCheck) {
                ...
                illegalOrphanMessages.add("This MicroMarket (" + microMarket + ") cannot be destroyed since the Customer " + customerListOrphanCheckCustomer + " in its customerList field has a non-nullable microMarket field.");
            }
            if (illegalOrphanMessages != null) {
                throw new IllegalOrphanException(illegalOrphanMessages);
            }
